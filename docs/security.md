...
| No command passthrough: `AnalysisRequest` rejects the denylist `command, argv, args, cmd, shell, exec, filter_complex, filter, api_key, token, env` **recursively over the whole request document** (case-insensitive, so a key nested in `parameters` or deeper is rejected, never stripped) plus any unknown field; parameters are validated against a per-kind schema (name, type, range); batch `budget` accepts only the three enforced names. The denylist is a superset of AI-video-production-OS `SKILL_SPEC.md` section 3.1 and is published in `contract --json` → `security.forbidden_keys` | `contract.py`, `budget.py`, `tests/test_contract.py` |
...
| Timeouts kill the whole process group (`start_new_session` + `killpg(SIGKILL)` on POSIX; `CREATE_NEW_PROCESS_GROUP` + `taskkill /T /F` on Windows); no partial observation, no cache entry is written (tested with a grandchild process and with real ffmpeg) | `runner.py`, `tests/test_contract.py`, `tests/test_integration.py` |
| stdout / stderr contract: with `--json` stdout carries exactly one response document, on success and failure alike, so an adapter parses JSON and never text; stderr is diagnostics only. JSON is emitted as UTF-8 bytes and stdin is decoded as UTF-8, bypassing the platform text encoding, so a non-ASCII path can never turn into a traceback instead of a document; human-readable output escapes unencodable characters rather than crashing | `cli.py`, `tests/test_integration.py::test_non_ascii_path_on_non_utf8_pipe` |
| `media-analysis conformance --json` submits real requests and inspects real source against AI-video-production-OS `SKILL_SPEC.md` section 8 (forbidden keys, shell-out, workspace confinement); a regression here is a self-detected FAIL, not just an assumption in the docs | `conformance.py` |
